...
def get_events(client: Client, after: str, before: str, next_page_number: int):
    ...
    """
    if not before:
        before = arg_to_datetime(arg="now", arg_name="before", required=True).strftime(
            "%Y-%m-%dT%H:%M:%SZ")  # type: ignore
    demisto.debug(
        f"Calling get_list_threats with: {after=}, {before=}, {next_page_number=}")
    next_page_number, threats_ids = get_list_threats(
        client, after, before, next_page_number)
    last_run = {'before': before, 'after': after,
                'next_page_number': next_page_number}
    messages = []
    if threats_ids:
        for threat in reversed(threats_ids):
            messages += format_messages(get_messages_by_datetime(
                client, threat.get("threatId"), after, before))
...
def get_list_threats(client: Client, after: str, before: str, next_page_number: int):
    ...
    while len(threats) < FETCH_LIMIT and is_next_page:
        ...
        demisto.debug(
            f"fetching events: epoch {next_page_number}, {page_size=}")
        params = assign_params(
            pageSize=page_size, filter=f"receivedTime gte {after} lte {before}", pageNumber=next_page_number)
        ...
        if res.get("nextPageNumber"):
            ...
            demisto.debug(
                f"There are more events to fetch. {next_page_number=}")
        else:
...
def main():  # pragma: no cover
    ...
    proxy = params["proxy"]
    after = arg_to_datetime(arg="1 minute").strftime(
        "%Y-%m-%dT%H:%M:%SZ")  # type: ignore
    before: str = ""
    next_page_number = 1
    client = Client(
        ...
        if command == "test-module":
            ...
            command_results = CommandResults(
                readable_output=tableToMarkdown(
                    f"{VENDOR} - {PRODUCT} events", threats),
