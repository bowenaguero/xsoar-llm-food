...
class Client(OktaClient):
    ...
    def get_group_id(self, group_name):
        ...
        res = self.http_request(
            method="GET", url_suffix=uri, params=query_params)
    ...
    def get_user_id(self, username):
        uri = "/api/v1/users"
        query_params = {"filter": encode_string_results(
            f'profile.login eq "{username}"')}
        res = self.http_request(
            method="GET", url_suffix=uri, params=query_params)
        if res and len(res) == 1:
            return res[0].get("id")
    ...
    def get_readable_groups(raw_groups):
        groups = []
        raw_groups = raw_groups if isinstance(
            raw_groups, list) else [raw_groups]
    ...
    def get_readable_logs(raw_logs):
        ...
        for log in raw_logs:
            ...
            time_published = datetime.strptime(
                log.get("published"), "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%m/%d/%Y, %H:%M:%S")
            log = {
                ...
                + (f": {log.get('outcome', {}).get('reason')}" if log.get("outcome",
                   {}).get("reason") else ""),
    ...
    def poll_verify_push(self, url):
        ...
        while counter < 10:
            response = self.http_request(
                method="GET", full_url=url, url_suffix="")
    ...
    def search(self, term, limit, advanced_search):
        uri = "/api/v1/users"
        query_params = assign_params(limit=limit, q=encode_string_results(
            term), search=encode_string_results(advanced_search))
    ...
    def create_user(self, cred, profile, group_ids, activate):
        body = {"profile": profile,
                "groupIds": group_ids or [], "credentials": cred}
        uri = "/api/v1/users"
    ...
    def build_credentials(args):
        ...
        if args.get("passwordQuestion") and args.get("passwordAnswer"):
            cred["recovery_question"] = {"question": args.get(
                "passwordQuestion"), "answer": args.get("passwordAnswer")}
        if args.get("providerName") and args.get("providerType"):
            cred["provider"] = {"name": args.get(
                "providerName"), "type": args.get("providerType")}
        return cred
    ...
    def get_paged_results(self, uri, query_param=None, max_limit=None):
        response = self.http_request(
            method="GET", url_suffix=uri, resp_type="response", params=query_param)
    ...
    def list_users(self, args):
        ...
        response = self.http_request(
            method="GET", url_suffix=uri, resp_type="response", params=query_params)
        paged_results = response.json()
        if limit > 200:
            query_params = {}
            limit -= 200
...
def get_user_factors_command(client, args):
    ...
    outputs = {"Account(val.ID && val.ID === obj.ID)": {
        "Factor": context, "ID": user_id}}
...
def set_password_command(client, args):
    ...
    if argToBoolean(args.get("temporary_password", False)):
        ...
        expire_password_readable_output = tableToMarkdown(
            "Okta Temporary Password", expire_password_response, removeNull=True)
...
def expire_password_command(client, args):
    ...
    readable_output = tableToMarkdown(
        "Okta Expired Password", raw_response, removeNull=True)
    outputs = {"Account(val.ID && val.ID === obj.ID)": createContext(
        user_context, removeNull=True)}
...
def remove_from_group_command(client, args):
    ...
    if (not (args.get("username") or user_id)) or (not (args.get("groupName") or group_id)):
        raise Exception(
            "You must supply either 'Username' or 'userId' and either 'groupName' or 'groupId'")
    if not user_id:
        user_id = client.get_user_id(args.get("username"))
    if not group_id:
        group_id = client.get_group_id(args.get("groupName"))
...
def get_groups_for_user_command(client, args):
    ...
    outputs = {"Account(val.ID && val.ID === obj.ID)": {
        "Group": context, "ID": user_id, "Type": "Okta"}}
...
def verify_push_factor_command(client, args):
    ...
    outputs = {"Account(val.ID && val.ID === obj.ID)": {
        "ID": user_id, "VerifyPushResult": poll_response.get("factorResult")}}
...
def search_command(client, args):
    ...
    if not term and not advanced_search:
        raise DemistoException(
            "Please provide either the term or advanced_search argument")
...
def get_user_command(client, args):
    if not (args.get("username") or args.get("userId")):
        raise Exception("You must supply either 'Username' or 'userId")
    user_term = args.get("userId") if args.get(
        "userId") else args.get("username")
...
def create_user_command(client, args):
    ...
    outputs = {
        "Account(val.ID && val.ID === obj.ID)": createContext(user_context)}
    readable_output = tableToMarkdown(
        f"Okta User Created: {args.get('login')}:", client.get_readable_users(raw_response))
    return (readable_output, outputs, raw_response)
...
def update_user_command(client, args):
    ...
    readable_output = tableToMarkdown(
        f"Okta user: {args.get('username')} Updated:", raw_response.get("profile"))
    return (readable_output, {}, raw_response)
...
def get_group_members_command(client, args):
    ...
    limit = args.get("limit")
    group_id = args.get("groupId") or client.get_group_id(
        args.get("groupName"))
    raw_members = client.get_group_members(group_id, limit)
    users_context = client.get_users_context(raw_members)
    users_readable = client.get_readable_users(
        raw_members, args.get("verbose"))
    context = createContext(users_context, removeNull=True)
    outputs = {"Account(val.ID && val.ID === obj.ID)": context}
...
def get_logs_command(client, args):
    ...
    outputs = {"Okta.Logs.Events(val.uuid && val.uuid === obj.uuid)": createContext(
        raw_response)}
    return (readable_output, outputs, raw_response)
...
def get_application_authentication_command(client, args):
    ...
    logs = client.get_readable_logs(raw_response)
    readable_output = tableToMarkdown(
        "Application Authentication Events", logs)
    outputs = {"Okta.Logs.Events(val.uuid && val.uuid === obj.uuid)": createContext(
        raw_response)}
    return (readable_output, outputs, raw_response)
...
def delete_user_command(client, args):
    ...
    if user.get("status") != "DEPROVISIONED":
        client.deactivate_user(
            args.get("userId") or client.get_user_id(args.get("username")))
...
def create_zone_command(client, args):
    ...
    zoneObject = {"name": "", "type": "IP",
                  "status": "ACTIVE", "gateways": [], "proxies": []}
    zoneObject = apply_zone_updates(
        zoneObject, zone_name, gateway_ips, proxies)
...
def update_zone_command(client, args):
    ...
    if zoneID == zoneObject.get("id"):
        ...
        zoneObject = apply_zone_updates(
            zoneObject, zoneName, gatewayIPs, proxyIPs, updateType)
...
def assign_group_to_app_command(client, args):
    ...
    if not group_id:
        ...
        if group_id is None:
            raise ValueError(
                "Either group name not found or multiple groups include this name.")
...
def main():
    try:
        ...
        client = Client(
            ...
            api_token=params.get("credentials", {}).get(
                "password") or params.get("apitoken"),
            auth_type=AuthType.OAUTH if argToBoolean(
                params.get("use_oauth", False)) else AuthType.API_TOKEN,
            client_id=params.get("client_id"),
            scopes=OAUTH_TOKEN_SCOPES,
            private_key=params.get("private_key"),
            jwt_algorithm=JWTAlgorithm(params["jwt_algorithm"]) if params.get(
                "jwt_algorithm") else None,
        ...
        if command in commands:
            ...
            if isinstance(result, CommandResults | str | dict):
                ...
                return_outputs(readable_output=human_readable,
                               outputs=outputs, raw_response=raw_response)
        ...
        return_error(
            f"Failed to execute {demisto.command()} command. Error: {e!s}")
