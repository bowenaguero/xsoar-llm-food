...
def create_subscription(args, client):
    try:
        attributes = {}
        kwargs = {"TopicArn": args.get(
            "topicArn"), "Protocol": args.get("protocol")}
        if args.get("endpoint") is not None:
            kwargs.update({"Endpoint": args.get("endpoint")})
        if args.get("returnSubscriptionArn") is not None:
            kwargs.update({"ReturnSubscriptionArn": bool(
                args.get("returnSubscriptionArn"))})
        ...
        if args.get("rawMessageDelivery") is not None:
            attributes.update(
                {"RawMessageDelivery": args.get("rawMessageDelivery")})
        ...
        if args.get("subscriptionRoleArn") is not None:
            attributes.update(
                {"SubscriptionRoleArn": args.get("subscriptionRoleArn")})
        if attributes:
...
def send_message(args, client):
    try:
        ...
        if args.get("messageDeduplicationId") is not None:
            kwargs.update(
                {"MessageDeduplicationId": args.get("messageDeduplicationId")})
...
def create_topic(args, client):
    try:
        ...
        if args.get("contentBasedDeduplication") is not None:
            attributes.update(
                {"ContentBasedDeduplication": args.get("contentBasedDeduplication")})
        if attributes:
            kwargs.update({"Attributes": attributes})
    ...
    try:
        ...
        if command == "test-module":
            ...
            raise NotImplementedError(
                f"{command} is not an existing AWS-SNS command")
