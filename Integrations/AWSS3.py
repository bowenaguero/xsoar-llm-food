...
def create_bucket_command(args: Dict[str, Any], aws_client: AWSClient) -> CommandResults:
    ...
    if args.get("locationConstraint") is not None:
        kwargs.update({"CreateBucketConfiguration": {
    ...
    data.append({"BucketName": args.get("bucket"),
...
def list_buckets_command(args: Dict[str, Any], aws_client: AWSClient) -> CommandResults:
    ...
    for bucket in response["Buckets"]:
        data.append(
            {"BucketName": bucket["Name"], "CreationDate": datetime.strftime(
...
def put_bucket_policy_command(args: Dict[str, Any], aws_client: AWSClient) -> CommandResults:
    ...
    kwargs = {"Bucket": args.get(
        "bucket", "").lower(), "Policy": args.get("policy")}
    if args.get("confirmRemoveSelfBucketAccess") is not None:
        kwargs.update({"ConfirmRemoveSelfBucketAccess": args.get(
...
def download_file_command(args: Dict[str, Any], aws_client: AWSClient):
    ...
    client.download_fileobj(
        args.get("bucket", "").lower(), args.get("key"), data)
...
def get_public_access_block(args: Dict[str, Any], aws_client: AWSClient) -> CommandResults:
    ...
    public_access_block_configuration = response.get(
...
def main():  # pragma: no cover
    ...
    aws_access_key_id = params.get("credentials", {}).get(
        "identifier") or params.get("access_key")
    aws_secret_access_key = params.get("credentials", {}).get(
        "password") or params.get("secret_key")
    ...
    try:
        ...
        validate_params(aws_default_region, aws_role_arn,
                        aws_role_session_name, aws_access_key_id, aws_secret_access_key)
