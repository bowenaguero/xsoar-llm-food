...
def parse_outputs(users_data):
    ...
    if isinstance(users_data, list):
        ...
        for user_data in users_data:
            user_readable = {camel_case_to_readable(
                k): v for k, v in user_data.items() if k != "@removed"}
            if "@removed" in user_data:
                user_readable["Status"] = "deleted"
            users_readable.append(user_readable)
            users_outputs.append(
                {k.replace(" ", ""): v for k, v in user_readable.copy().items()})

        return users_readable, users_outputs

    else:
        user_readable = {camel_case_to_readable(
            k): v for k, v in users_data.items() if k != "@removed"}
        if "@removed" in users_data:
            user_readable["Status"] = "deleted"
        user_outputs = {k.replace(" ", ""): v for k,
                        v in user_readable.copy().items()}
...
class MsGraphClient:
    ...
    def disable_user_account_session(self, user):
        self.ms_client.http_request(
            method="PATCH", url_suffix=f"users/{quote(user)}", data=BLOCK_ACCOUNT_JSON, resp_type="text")
    ...
    def delete_user(self, user):
        self.ms_client.http_request(
            method="DELETE", url_suffix=f"users/{quote(user)}", resp_type="text")

    def create_user(self, properties):
        self.ms_client.http_request(
            method="POST", url_suffix="users", json_data=properties)
    ...
    def update_user(self, user: str, updated_fields: str, delimiter: str = ","):
        ...
        for key_value in updated_fields.split(delimiter):
            ...
            body[field] = value
        self.ms_client.http_request(
            method="PATCH", url_suffix=f"users/{quote(user)}", json_data=body, resp_type="text")
    ...
    def get_delta(self, properties):
        users = self.ms_client.http_request(
            method="GET", url_suffix="users/delta", params={"$select": properties})
    ...
    def list_users(self, properties, page_url, filters):
        if page_url:
            response = self.ms_client.http_request(
                method="GET", url_suffix="users", full_url=page_url)
        else:
            response = self.ms_client.http_request(
                method="GET",
                url_suffix="users",
    ...
    def get_direct_reports(self, user):
        res = self.ms_client.http_request(
            method="GET", url_suffix=f"users/{quote(user)}/directReports")
    ...
    def get_manager(self, user):
        manager_data = self.ms_client.http_request(
            method="GET", url_suffix=f"users/{quote(user)}/manager")
    ...
    def revoke_user_session(self, user):
        self.ms_client.http_request(
            method="POST", url_suffix=f"users/{quote(user)}/revokeSignInSessions", resp_type="text")
    ...
    def delete_tap_policy(self, user_id, policy_id):
        ...
        self.ms_client.http_request(
            method="DELETE", url_suffix=url_suffix, resp_type="text")
...
def create_user_command(client: MsGraphClient, args: dict):
    ...
    human_readable = tableToMarkdown(
        name=f"{user} was created successfully:", t=user_readable, removeNull=True)
    accounts = create_account_outputs(user_outputs)
    outputs = {"MSGraphUser": user_outputs,
...
def change_password_user_command(client: MsGraphClient, args: dict):
    ...
    force_change_password_next_sign_in = args.get(
        "force_change_password_next_sign_in", "true") == "true"
    force_change_password_with_mfa = args.get(
        "force_change_password_with_mfa", False) == "true"

    client.password_change_user(
        user, password, force_change_password_next_sign_in, force_change_password_with_mfa)
...
def get_delta_command(client: MsGraphClient, args: dict):
    ...
    headers = list(set([camel_case_to_readable(p) for p in argToList(
        properties)] + ["ID", "User Principal Name"]))

    users_readable, users_outputs = parse_outputs(users_data)
    human_readable = tableToMarkdown(
        name="All Graph Users", headers=headers, t=users_readable, removeNull=True)
...
def get_user_command(client: MsGraphClient, args: dict):
    ...
    accounts = create_account_outputs(user_outputs)
    human_readable = tableToMarkdown(
        name=f"{user} data", t=user_readable, removeNull=True)
    outputs = {"MSGraphUser": user_outputs,
               "Account": accounts[0] if accounts else []}
...
def list_users_command(client: MsGraphClient, args: dict):
    properties = args.get(
        "properties", "id,displayName,jobTitle,mobilePhone,mail")
    next_page = args.get("next_page", None)
    filters = args.get("filter", None)
    users_data, result_next_page = client.list_users(
        properties, next_page, filters)
    users_readable, users_outputs = parse_outputs(users_data)
    ...
    human_readable = tableToMarkdown(
        name="All Graph Users", t=users_readable, removeNull=True, metadata=metadata)
...
def get_direct_reports_command(client: MsGraphClient, args: dict):
    ...
    human_readable = tableToMarkdown(
        name=f"{user} - direct reports", t=reports_readable, removeNull=True)
...
def get_manager_command(client: MsGraphClient, args: dict):
    ...
    human_readable = tableToMarkdown(
        name=f"{user} - manager", t=manager_readable, removeNull=True)
...
def list_tap_policy_command(client: MsGraphClient, args: dict) -> CommandResults:
    ...
    headers = ["Policy ID", "Start Date Time", "Lifetime In Minutes",
               "Is Usable Once", "Is Usable", "Method Usability Reason"]
    human_readable = tableToMarkdown(
        name=f"TAP Policy for User ID {user_id}:", headers=headers, t=tap_readable_dict)
...
def create_tap_policy_command(client: MsGraphClient, args: dict) -> CommandResults:
    ...
    create_zip_with_password(
        generated_tap_password=generated_password, zip_password=zip_password)
...
def create_zip_with_password(generated_tap_password: str, zip_password: str):
    ...
    try:
        ...
        file_res = generate_password_protected_zip(
            zip_file_name, zip_password, generated_tap_password)

    except Exception as e:
        raise DemistoException(
            f"Could not generate zip file. Error:\n{str(e)}")
...
def main():
    ...
    url = urljoin(
        azure_cloud.endpoints.microsoft_graph_resource_id, f"/{API_VERSION}/")
    tenant = params.get("creds_tenant_id", {}).get(
        "password", "") or params.get("tenant_id", "")
    auth_and_token_url = params.get("creds_auth_id", {}).get(
        "password", "") or params.get("auth_id", "")
    enc_key = params.get("creds_enc_key", {}).get(
        "password", "") or params.get("enc_key", "")
    verify = not params.get("insecure", False)
    redirect_uri = params.get("redirect_uri", "")
    auth_code = params.get("creds_auth_code", {}).get(
        "password", "") or params.get("auth_code", "")
    proxy = params.get("proxy", False)
    handle_error = argToBoolean(params.get("handle_error", "true"))
    certificate_thumbprint = params.get("creds_certificate", {}).get(
    ...
    managed_identities_client_id = get_azure_managed_identities_client_id(
        params)
    self_deployed: bool = params.get(
        "self_deployed", False) or managed_identities_client_id is not None
