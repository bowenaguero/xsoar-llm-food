...
def create_blocks(text: str, entitlement: str, options: list, reply: str) -> list:
    ...
    blocks: list = [{"type": "section", "text": {
        "type": "mrkdwn", "text": text}}]

    elements = []
    for option in options:
        element = {"type": "button", "text": {"type": "plain_text",
                                              "emoji": True, "text": option["text"]}, "value": value}
        if "style" in option:
...
def main():
    ...
    if response_type == "thread":
        ...
        blocks = json.dumps(create_blocks(
            demisto.args()["message"], entitlement_string, options, reply))
    ...
    try:
        ...
        if "Unsupported Command" in str(e):
            ...
            return_error(
                "An error has occurred while executing the send-notification command", error=e)
